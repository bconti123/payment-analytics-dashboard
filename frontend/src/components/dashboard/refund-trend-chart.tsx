...
import { getRefundTrend } from "@/lib/api"
import type { Interval, RefundTrendPoint } from "@/lib/api/schemas"
...
function toChartPoints(
  ...
  return points.map((p) => ({
    ...
    refundAmount: Number(p.refund_amount),
    count: p.count,
  }))
}
...
export function RefundTrendChart() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["dashboard", "refundTrend", interval],
    queryFn: () => getRefundTrend({ interval }),
  })

  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <CardTitle className="text-base font-medium">Refund trend</CardTitle>
        <div
          role="tablist"
          aria-label="Interval"
  ...
  return (
    <div className="rounded-md border bg-popover px-3 py-2 text-xs shadow-md">
      ...
      <div className="mt-1 text-foreground">
        {formatCurrency(point.refundAmount)}
      </div>
      <div className="text-muted-foreground">
        {point.count} refund{point.count === 1 ? "" : "s"}
      </div>
    </div>
  )
}
